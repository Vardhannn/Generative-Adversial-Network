...
def build_xception_feature_extractor(hr_shape):
    xception_model = Xception(weights="imagenet", include_top=False, input_shape=hr_shape)
...
hr_shape = (128, 128, 3)
vit_model = build_xception_feature_extractor(hr_shape)
vit_model.summary()
vit_model.trainable = False
# %%

gan_model = create_comb(generator, discriminator, vit_model, lr_ip, hr_ip)
...
if __name__ == "__main__":
    ...
    visualize_feature_maps(vit_model, layer_names, input_image, num_feature_maps=16)
...
def extract_feature_maps(xception_model, input_image, layer_name="block1_conv1"):
    submodel = Model(inputs=xception_model.input, outputs=xception_model.get_layer(layer_name).output)
    feature_maps = submodel.predict(input_image)
    return feature_maps
...
def visualize_feature_maps(feature_maps, num_feature_maps=16):
    ...
    for i in range(min(num_feature_maps, feature_maps.shape[-1])):
        ...
        plt.axis('off')
    plt.suptitle(f"Feature Maps from Layer {i}")
    plt.show()

if __name__ == "__main__":
    hr_shape = (128, 128, 3)
    xception_model = build_xception_feature_extractor(hr_shape)
    xception_model.summary()
    input_image = np.random.rand(1, 128, 128, 3)
    layer_name = "block1_conv1"  # First convolutional layer
    feature_maps = extract_feature_maps(xception_model, input_image, layer_name=layer_name)
    visualize_feature_maps(feature_maps, num_feature_maps=16)
# %%
